...
func _process(delta: float) -> void:
	...
	if $"../tiempo".value < $"../tiempo".max_value:
		if Escudo <= 0:
			...
			if Input.is_action_just_pressed("Medio"):
				Escudo = 2
			...
			if Input.is_action_just_pressed("Arriba") and not Escudo == 1:
				Espada = 1
			if Input.is_action_just_pressed("Medio") and not Escudo == 2:
				Espada = 2
			if Input.is_action_just_pressed("Abajo") and not Escudo == 3:
				Espada = 3
		elif  Espada > 0:
